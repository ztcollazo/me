  ...
  <li style={{ transitionDuration: '.5s' }} className="rounded-xl text-left cursor-pointer w-full md:flex-grow flex flex-col card justify-between bg-white dark:bg-gray-700 hover:translate-x-[1px] hover:translate-y-[-1px] transition-transform">
    <div className="flex flex-row justify-between">
      ...
      <span className="flex flex-col">
        {project.homepageUrl && <a target="_blank" rel="noopener noreferrer" aria-label="Open page" className="inline-block m-2 rounded-full border-[1px] border-transparent hover:border-current p-2" href={['http', 'https', '//'].some((s) => project.homepageUrl.startsWith(s)) ? project.homepageUrl : 'https://'.concat(project.homepageUrl)}><HiOutlineExternalLink size={18} /></a>}
        {!project.isPrivate && <a target="_blank" rel="noopener noreferrer" aria-label="Open github repository" className="inline-block m-2 rounded-full border-[1px] border-transparent hover:border-current p-2" href={project.url}><RiGithubLine size={18} /></a>}
      </span>
    </div>
